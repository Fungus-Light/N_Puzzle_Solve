...
AStar::AStar(int w, int h, int m, int *s, int *g)
{
    IsPrintResult = false;
...
void AStar::SetPrintState(bool v)
{
    IsPrintResult = v;
}

void AStar::InsertOpen(Status *s)
{
    if (open.empty())
    {
        open.push_back(*s);
    }
    if (Status::AStartValue(s, &Goal) > Status::AStartValue(&open.back(), &Goal))
    {
        open.push_back(*s);
        return;
    }
    ...
    {
        if (Status::AStartValue(s, &Goal) <= Status::AStartValue(&(*i), &Goal))
        {
            open.insert(i, *s);
            return;
        }
